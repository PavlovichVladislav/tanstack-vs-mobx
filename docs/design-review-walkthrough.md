...
- Шаблон, стандартизация mobx стора
...
- различается не transport и не UI, а способ управления серверным состоянием на клиенте
- компонентная декомпозиция у веток одинаковая

### Что объяснить команде

- сравнение честное, потому что бизнес логика одна и та же
...
Вообще можно руками напилить свой хук/метод, который инкапсулирует управление жизненным циклом - статусы + контроллер и не трогать танстак. Минус только, что это велосипед. Т.е. есть полноценное развивающее решение, мы тянем его часть и что - то допиливаем. Так в свое время делал Алексей Анисимов. Но для условно мелкого виджета мб оправдано, на большие проекты я бы танстак тащил.
...
- “Если мы остаемся на MobX-only, то можно выделить какую - то шаблонную часть”
...
- shared cache
- единая модель жц запроса
- меньше ручного управления кэшем

Где TanStack выглядит слабее или сложнее:

- выше порог входа (спорно, можем пилить свое решение и переносить его или делать везде по-разному)
- data flow менее линейный
- нужно понимать query keys, invalidation, query cache semantics

Вопрос команде:

- “Для нас важнее локальная явность в одном store или единый стандартизированный жизненный цикл запросов?”
...
- меньше ручного управления состоянием запросов
- policy по stale/refetch живет централизованно

### TanStack: какие trade-offs появляются

- нужно понимать query key
- загрузка списка “размазана” между hook и потребителями (один хук дергается отовсюду)
...
- несколько компонентов могут вызывать одни и те же query hooks (и это не факт, что плюс)
